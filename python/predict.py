...
MODEL_PATH = os.path.join(MODEL_DIR, 'pipe_joblib.pkl')
...
def predict():
    try:
        ...
        # Handle special cases (wickets left or balls left)
        if wickets_remaining == 0 or (runs_left > 1 and balls_left == 0):
            return jsonify({
                "batting_team": {"team_name": batting_team, "winning_probability": 0.0},
                "bowling_team": {"team_name": bowling_team, "winning_probability": 100.0}
            })
        if balls_left == 0 and runs_left == 1:
            return jsonify({
                "batting_team": {"team_name": batting_team, "winning_probability": 50.0},
                "bowling_team": {"team_name": bowling_team, "winning_probability": 50.0}
            })
        if runs_left == 0 and balls_left > 0:
            return jsonify({
                "batting_team": {"team_name": batting_team, "winning_probability": 100.0},
                "bowling_team": {"team_name": bowling_team, "winning_probability": 0.0}
            })
        ...
        try:
            ...
            batting_prob = round(lr_prediction[1] * 100, 2)
            bowling_prob = round(lr_prediction[0] * 100, 2)
        except ValueError as e:
            # This handles unknown categories gracefully
            print(f"Unknown category encountered: {e}")
            batting_prob = 50.0
            bowling_prob = 50.0
        ...
        response = {
            "batting_team": {
                ...
                "winning_probability": batting_prob
            },
            "bowling_team": {
                "team_name": bowling_team,
                "winning_probability": bowling_prob
            }
        }
